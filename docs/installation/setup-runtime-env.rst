...
1. Download installation package from `the download center <https://www.hiascend.com/cann/download>`__
   (recommended version is :code:`9.0.0-beta.1`) **depending on the host platform**:

   .. code:: bash

      wget https://ascend-repo.obs.cn-east-2.myhuaweicloud.com/CANN/CANN%209.0.T2/Ascend-cann_9.0.0-beta.1_linux-$(arch).run

2. Install downloaded package with the following command:

   .. code:: bash

      bash Ascend-cann_9.0.0-beta.1_linux-$(arch).run --full
