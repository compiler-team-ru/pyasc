...
{
    return builder.create<ascendc::UpdateMaskOp>(builder.getUnknownLoc(), builder.getType<ascendc::MaskRegType>(),
                                                 count, type);
}
  ...
  public:
    ...
    auto binary()
    {
        return [&](ascendc::BinaryL2Op binaryOp) {
            ...
            builder.create<T>(builder.getUnknownLoc(), regTensors[binaryOp.getDst()], regTensors[binaryOp.getSrc0()],
    ...
    auto unary()
    {
        return [&](ascendc::UnaryL2Op unaryOp) {
            ...
            builder.create<T>(builder.getUnknownLoc(), regTensors[unaryOp.getDst()], regTensors[unaryOp.getSrc()],
                              mask);
            unaryOp.erase();
        };
    }
...
void processGroup(OpBuilder &builder, OpGroup &group, DominanceInfo &di)
{
    ...
    auto div = builder.create<arith::DivSIOp>(builder.getUnknownLoc(), builder.getIndexType(),
                                              getVecLenIndex.getResult(), sizeIndex);
    ...
    auto repeatTimes = builder.create<arith::CeilDivSIOp>(builder.getUnknownLoc(), builder.getIndexType(),
                                                          calCountIndex, oneRepeatSizeIndex);
    ...
    for (auto &value : inputTensors) {
        ...
        auto srcAddr = builder.create<emitasc::PtrOffsetOp>(builder.getUnknownLoc(), resultType, addrTensors[value],
                                                            nullptr, mu.getResult());
    ...
    for (auto &value : outputTensors) {
        auto resultType =
            MemRefType::get(getShape(value), groupType, {}, static_cast<int>(ascendc::AddressSpace::ubuf));
        auto dstAddr = builder.create<emitasc::PtrOffsetOp>(builder.getUnknownLoc(), resultType, addrTensors[value],
                                                            nullptr, mu.getResult());
...
namespace ascendc {
std::unique_ptr<Pass> createFuseVFBlockPass()
{
    return std::make_unique<FuseVFBlockPass>();
}
} // namespace ascendc
} // namespace mlir
