...
class Promoter {
    ...
    void promote(Operation* op)
    {
        ...
        llvm::copy_if(op->getOperands(), std::back_inserter(opndInBlock), [block](Value operand) {
            return operand.getDefiningOp() && block == operand.getParentBlock();
        });
...
std::unique_ptr<Pass> mlir::asctile::createPromotePureOpsPass() { return std::make_unique<PromotePureOpsPass>(); }
