...
bool processMatmulAcc(asctile::MatmulOp matmulOp)
{
    ...
    if (auto constOp = acc.getDefiningOp<arith::ConstantOp>()) {
        ...
        auto newForOp = builder.create<scf::ForOp>(forOp.getLoc(), forOp.getLowerBound(), forOp.getUpperBound(),
...
std::unique_ptr<Pass> mlir::asctile::createLegalizeMatmulPass()
{
    return std::make_unique<LegalizeMatmulPass>();
}
