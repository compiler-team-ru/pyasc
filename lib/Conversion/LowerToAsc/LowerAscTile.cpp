        ...
        if (opType.getLoc() == asctile::TileLocation::L0A || opType.getLoc() == asctile::TileLocation::L0B) {
            ...
            auto nd2NzParams = rewriter.create<ascendc::ConstructOp>(
                ...
                ValueRange {const1, srcTensorShape[0], srcTensorShape[1], const0, srcTensorShape[1], srcTensorShape[0],
                            const1, const0});
            auto l1Dst = createTensorOp(rewriter, loc, opType,
                                        asctile::TileLocationAttr::get(op.getContext(), asctile::TileLocation::L1));
            ...
            rewriter.create<emitasc::SetMemberOp>(loc, loadDataParams, "ifTranspose",
                                                  consts.i1(opType.getLoc() == asctile::TileLocation::L0B));
...
struct LowerAscTilePass : public asclower::impl::LowerAscTileBase<LowerAscTilePass> {
    void runOnOperation() override
    {
        ...
        patterns.insert<
            //
            ConvertTensor, ConvertLoad, ConvertGetValue, ConvertStore, ConvertSetValue, ConvertSplat, ConvertRelu, ConvertCast, ConvertSelect,
