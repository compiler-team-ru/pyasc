...
struct ConvertCmpI : public ConvertOp<arith::CmpIOp> {
    ...
    LogicalResult convert(arith::CmpIOp op, ConvertRewriter &rewriter) const override
    {
        ...
        rewriter.create<ascendc::CompareL0Op>(loc, dst, src0, src1, cmpMode,
                                              rewriter.create<emitasc::MaskOp>(loc, zero, zero), zero, repeatParams);
...
std::unique_ptr<Pass> mlir::asclower::createLowerArithI1Pass()
{
    return std::make_unique<LowerArithI1Pass>();
}
