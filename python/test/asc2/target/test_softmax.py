...
def softmax(input_ptr: asc2.GlobalAddress, output_ptr: asc2.GlobalAddress, input_num_rows: asc2.ConstExpr,
    ...
    for i in range(asc2.block_idx(), input_num_rows, asc2.block_num(), unroll_factor=unroll_factor, parallel=True):
        row = asc2.load(in_gm, [1, tile_shape[1]], real_shape=[1, input_num_cols], offsets=[i, 0],
                        pad_value=float('-inf'))
        row_minus_max = row - asc2.reduce_max(row)
        numerator = asc2.exp(row_minus_max)
        denominator = asc2.reduce_sum(numerator)
        out = numerator / denominator
        asc2.store(out, out_gm, real_shape=[1, input_num_cols], offsets=[i, 0])
...
@pytest.mark.parametrize("block_num, unroll_factor, input_shape, in_out_dtype, tiling_key, tiling_values", [
    ...
    (56, 2, [4, 16, 128, 128], torch.float32, 1000, [8192, 128, 128, 98, 147, 2]),
    (56, 2, [32, 400, 30], torch.float32, 1000, [12800, 30, 32, 229, 229, 1]),
    (56, 2, [1, 12, 256, 256], torch.float32, 1000, [3072, 256, 256, 49, 55, 4]),
    (12, 2, [2048, 7, 7], torch.float32, 500, [14336, 7, 12, 1, 1280, 256, 8, 8]),
