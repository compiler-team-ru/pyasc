...
def softmax_1d_kernel(dst_ptr, src_ptr, length: asc2.ConstExpr) -> None:
    ...
    src_tile = asc2.copy_in(src, [0], [length])
    dst_tile = asc2.softmax(src_tile)
    asc2.copy_out(dst_tile, dst, [0])
    asc2.copy_out(src_tile, src, [0])
...
def softmax_2d_kernel(dst_ptr, src_ptr, shape: asc2.ConstExpr) -> None:
    ...
    src_tile = asc2.copy_in(src, [0, 0], shape)
    dst_tile = asc2.softmax(src_tile)
    asc2.copy_out(dst_tile, dst, [0, 0])
    asc2.copy_out(src_tile, src, [0, 0])
