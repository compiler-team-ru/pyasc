...
def load_real_shape_1d_kernel(x_ptr: asc2.GlobalAddress, out_ptr: asc2.GlobalAddress, size: int,
    ...
    tile = asc2.copy_in(x_gm, [offset], [tile_size], real_shape=[real_size], pad_value=float('-inf'))
...
def test_load_real_shape_1d(kernel_type, size, tile_size, real_size, offset):
    ...
    real_size_arg = asc2.ConstExpr(real_size) if kernel_type == STATIC else real_size
...
def load_real_shape_2d_kernel(x_ptr: asc2.GlobalAddress, out_ptr: asc2.GlobalAddress, rows: int, cols: int,
    ...
    out_gm = asc2.global_tensor(out_ptr, [1])
    tile = asc2.copy_in(x_gm, offsets, tile_shape, real_shape=[real_rows, real_cols], pad_value=float('-inf'))
    max_val = asc2.reduce_max(tile)
    result = asc2.full([1], max_val, dtype=tile.dtype)
    asc2.copy_out(result, out_gm, [0])
...
def test_load_real_shape_2d(require_c310, kernel_type, shape, tile_shape, real_shape, offsets):
    ...
    out = torch.empty(1, dtype=torch.float32)
    real_rows_arg = asc2.ConstExpr(real_rows) if kernel_type == STATIC else real_rows
    real_cols_arg = asc2.ConstExpr(real_cols) if kernel_type == STATIC else real_cols
...
def store_real_shape_2d_kernel(x_ptr: asc2.GlobalAddress, y_ptr: asc2.GlobalAddress, out_ptr: asc2.GlobalAddress,
    ...
    x_tile = asc2.copy_in(x_gm, offsets, tile_shape)
    y_tile = asc2.copy_in(y_gm, offsets, tile_shape)
    result = x_tile + y_tile
    asc2.copy_out(result, out_gm, [0, 0], real_shape=[real_rows, real_cols])
