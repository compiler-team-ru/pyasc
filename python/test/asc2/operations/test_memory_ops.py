...
def kernel_static(x_ptr, y_ptr, z_ptr, tensor_shape: asc2.ConstExpr, tile_shape: asc2.ConstExpr,
    ...
    xt = asc2.copy_in(asc2.global_tensor(x_ptr, tensor_shape), offsets, tile_shape)
    yt = asc2.copy_in(asc2.global_tensor(y_ptr, tensor_shape), offsets, tile_shape)
    zt = xt + yt
    asc2.copy_out(zt, asc2.global_tensor(z_ptr, tensor_shape), offsets)


@asc2.jit(always_compile=True)
def kernel_dynamic_1D(x_ptr, y_ptr, z_ptr, ts0, tile_shape: asc2.ConstExpr, offsets: asc2.ConstExpr) -> None:
    xt = asc2.copy_in(asc2.global_tensor(x_ptr, [ts0]), offsets, tile_shape)
    yt = asc2.copy_in(asc2.global_tensor(y_ptr, [ts0]), offsets, tile_shape)
    zt = xt + yt
    asc2.copy_out(zt, asc2.global_tensor(z_ptr, [ts0]), offsets)


@asc2.jit(always_compile=True)
def kernel_dynamic_2D(x_ptr, y_ptr, z_ptr, ts0, ts1, tile_shape: asc2.ConstExpr, offsets: asc2.ConstExpr) -> None:
    xt = asc2.copy_in(asc2.global_tensor(x_ptr, [ts0, ts1]), offsets, tile_shape)
    yt = asc2.copy_in(asc2.global_tensor(y_ptr, [ts0, ts1]), offsets, tile_shape)
    zt = xt + yt
    asc2.copy_out(zt, asc2.global_tensor(z_ptr, [ts0, ts1]), offsets)


@asc2.jit(always_compile=True)
def kernel_scalar_load_store(x_ptr, y_ptr, z_ptr, tensor_shape: asc2.ConstExpr, offsets: asc2.ConstExpr) -> None:
    xt = asc2.copy_in(asc2.global_tensor(x_ptr, tensor_shape), offsets)
    yt = asc2.copy_in(asc2.global_tensor(y_ptr, tensor_shape), offsets)
    zt = xt + yt
    asc2.copy_out(zt, asc2.global_tensor(z_ptr, tensor_shape), offsets)
...
def test_store_1elem_tile(tensor_shape, offsets):
    ...
    def kernel(x_ptr, y_ptr, tensor_shape: asc2.ConstExpr, offsets: asc2.ConstExpr):
        ...
        s = asc2.copy_in(x, offsets)
        y = asc2.global_tensor(y_ptr, tensor_shape)
        asc2.copy_out(asc2.full([1], s), y, offsets)
...
def kernel_load_padding(x_ptr, out_ptr, input_shape: asc2.ConstExpr, tile_shape: asc2.ConstExpr,
    ...
    tile = asc2.copy_in(x_gm, offsets, tile_shape, pad_value=pad_value)
