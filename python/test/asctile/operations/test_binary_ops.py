...
binary_ops = [
    (asctile.add, torch.add, all_formats,
     [torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32]),
    (asctile.div, torch.div, all_formats, [torch.int16, torch.int32, torch.int64, torch.float16, torch.float32]),
    (asctile.mul, torch.mul, all_formats,
     [torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32]),
    (asctile.sub, torch.sub, all_formats,
     [torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32]),
    (asctile.bitwise_and, torch.bitwise_and, all_formats, [torch.int8, torch.int16, torch.int32, torch.int64]),
    (asctile.bitwise_or, torch.bitwise_or, all_formats, [torch.int8, torch.int16, torch.int32, torch.int64]),
    (asctile.bitwise_xor, torch.bitwise_xor, all_formats, [torch.int8, torch.int16, torch.int32, torch.int64]),
    (asctile.left_shift, torch.bitwise_left_shift, [VS], [torch.int16, torch.int32, torch.int64]),
    (asctile.right_shift, torch.bitwise_right_shift, [VS], [torch.int16, torch.int32, torch.int64]),
    (asctile.maximum, torch.maximum, all_formats,
     [torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32]),
    (asctile.minimum, torch.minimum, all_formats,
     [torch.int16, torch.int32, torch.int64, torch.float16, torch.bfloat16, torch.float32]),
    (asctile.pow, torch.pow, all_formats, [torch.int8, torch.int16, torch.int32, torch.float16, torch.float32]),
]


@asctile.jit(always_compile=True)
def kernel(x_ptr, y_ptr, z_ptr, block_length: asctile.ConstExpr, fmt: asctile.ConstExpr, op: asctile.ConstExpr,
           mask_type: asctile.ConstExpr, count: asctile.ConstExpr, other: asctile.ConstExpr, hibits: asctile.ConstExpr,
           lowbits: asctile.ConstExpr) -> None:
    if fmt == VV:
        xt = asctile.copy_in(asctile.global_tensor(x_ptr, [32]), [0], [block_length])
        yt = asctile.copy_in(asctile.global_tensor(y_ptr, [32]), [0], [block_length])
    elif fmt == VS:
        xt = asctile.copy_in(asctile.global_tensor(x_ptr, [32]), [0], [block_length])
        yt = asctile.copy_in(asctile.global_tensor(y_ptr, [1]), [0])
    elif fmt == SV:
        xt = asctile.copy_in(asctile.global_tensor(x_ptr, [1]), [0])
        yt = asctile.copy_in(asctile.global_tensor(y_ptr, [32]), [0], [block_length])
    ...
    if mask_type == NO_MASK:
        ...
        with asctile.mask(count=count, other=other):
            zt = op(xt, yt)
            asctile.copy_out(zt, asctile.global_tensor(z_ptr, [32]), [0])
    elif mask_type == BIT_MASK:
        with asctile.mask(bits=[hibits, lowbits], other=other):
            zt = op(xt, yt)
            asctile.copy_out(zt, asctile.global_tensor(z_ptr, [32]), [0])
...
def test_binary_operations(require_c310, asc_op, torch_op, fmt, dtype, mask_type):
    if any((
            ...
            asc_op is asctile.div and not dtype.is_floating_point,
            asc_op is asctile.pow and dtype == torch.int16,
            asc_op in (asctile.bitwise_and, asctile.bitwise_or, asctile.bitwise_xor) and dtype != torch.int16,
    )):
        require_c310()
    if asc_op is asctile.pow and mask_type != NO_MASK:
        pytest.skip("asctile.pow doesn't support masking")
