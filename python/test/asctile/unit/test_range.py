...
class TestRange:
    ...
    def test_unroll_factor(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel():
            for _ in asctile.range(10, unroll_factor=4):
                x = zero_tile([128], asctile.float32)
                x + 1.0
    ...
    def test_gm_barrier(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel():
            for _ in asctile.range(10, gm_barrier=True):
                x = zero_tile([128], asctile.float32)
                x + 1.0
    ...
    def test_runtime_bound(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel(n: int):
            for _ in asctile.range(n):
                x = zero_tile([128], asctile.float32)
                x + 1.0

        kernel[1](MockValue(asctile.int32))
        assert mock_launch.call_count == 1
    ...
    def test_invalid_unroll_factor_type(self, jit_test):
        ...
        @jit_test
        def kernel():
            for _ in asctile.range(10, unroll_factor="invalid"):
                pass
    ...
    def test_invalid_gm_barrier_type(self, jit_test):
        ...
        @jit_test
        def kernel():
            for _ in asctile.range(10, gm_barrier="invalid"):
                pass
    ...
    def test_invalid_unroll_factor_zero(self, jit_test):
        ...
        @jit_test
        def kernel():
            for _ in asctile.range(10, unroll_factor=0):
                pass
...
class TestStaticRange:
    ...
    def test_single_arg(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel():
            for _ in asctile.static_range(10):
                x = zero_tile([128], asctile.float32)
                x + 1.0
    ...
    def test_start_stop(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel():
            for _ in asctile.static_range(2, 10):
                x = zero_tile([128], asctile.float32)
                x + 1.0
    ...
    def test_start_stop_step(self, jit_test, mock_launch, zero_tile):
        ...
        @jit_test
        def kernel():
            for _ in asctile.static_range(0, 10, 2):
                x = zero_tile([128], asctile.float32)
                x + 1.0
    ...
    def test_runtime_bound_fails(self, jit_test):
        ...
        @jit_test
        def kernel(n: int):
            for _ in asctile.static_range(n):
                pass
    ...
    def test_too_many_args(self, jit_test):
        ...
        @jit_test
        def kernel():
            for _ in asctile.static_range(1, 2, 3, 4):
                pass
