...
valid_src_dtypes = (asctile.int16, asctile.int32, asctile.float16, asctile.bfloat16, asctile.float32)
...
class TestWhere:
    ...
    def test_invalid_mask_dtype(self, jit_test, zero_tile):
        ...
        @jit_test
        def kernel():
            mask = zero_tile([32, 64], asctile.float32)
            src0 = zero_tile([32, 64], asctile.float32)
            src1 = zero_tile([32, 64], asctile.float32)
            asctile.where(mask, src0, src1)
    ...
    def test_invalid_src_dtype(self, jit_test, zero_tile):
        ...
        @jit_test
        def kernel():
            a = zero_tile([32, 64], asctile.int8)
            b = zero_tile([32, 64], asctile.int8)
            mask = a > b
            asctile.where(mask, a, b)
    ...
    def test_invalid_mask_type(self, jit_test, zero_tile):
        ...
        def kernel():
            src0 = zero_tile([32, 64], asctile.float32)
            src1 = zero_tile([32, 64], asctile.float32)
            asctile.where("invalid", src0, src1)
    ...
    def test_invalid_src0_type(self, jit_test, zero_tile):
        ...
        @jit_test
        def kernel():
            a = zero_tile([32, 64], asctile.float32)
            b = zero_tile([32, 64], asctile.float32)
            mask = a > b
            asctile.where(mask, "invalid", b)
...
class TestMask:
    ...
    def test_bits(self, jit_test, mock_launch, zero_tile):
        ...
        def kernel():
            x = zero_tile([32, 64], asctile.float32)
            y = zero_tile([32, 64], asctile.float32)
            with asctile.mask(bits=[0, 64], other=-1):
                x + y
    ...
    def test_bits_wrong_count(self, jit_test, zero_tile):
        ...
        def kernel():
            x = zero_tile([32, 64], asctile.float32)
            with asctile.mask(bits=[0, 64, 128]):
                x + 1
    ...
    def test_both_count_and_bits(self, jit_test, zero_tile):
        ...
        def kernel():
            x = zero_tile([32, 64], asctile.float32)
            with asctile.mask(count=8, bits=[0, 64]):
                x + 1
    ...
    def test_invalid_other_type(self, jit_test, zero_tile):
        ...
        def kernel():
            x = zero_tile([32, 64], asctile.float32)
            with asctile.mask(count=8, other="invalid"):
                x + 1
