...
def set_docstring(name: str, support_dtypes: Tuple[DataType], support_scalar: bool = False) -> Callable[[T], T]:
    ...
    examples = f"""
        Compute the element-wise {name} of all tensor elements: ::
            ...
            input = asctile.copy_in(tensor, [0, 0], [128, 256])
            result = asctile.{{fn_name}}(input)
        """
    if support_scalar:
        examples += f"""
        Compute the {name} of given scalar value: ::

            result = asctile.{{fn_name}}(1.0)
        """
...
def softmax(input: LocalTensor) -> LocalTensor:
    ...
    Examples:
        Compute row-wise softmax for a 2D tensor: ::
            ...
            input = asctile.copy_in(x_gm, [0, 0], [64, 1024])
            result = asctile.softmax(input)  # softmax applied independently to each of 64 rows

        Compute softmax for a 1D tensor: ::

            input = asctile.copy_in(x_gm, [0], [1024])
            result = asctile.softmax(input)  # softmax applied over all 1024 elements
    """
...
def rms_norm(input: LocalTensor, gamma: LocalTensor, epsilon: RuntimeFloat) -> LocalTensor:
    ...
    Examples:
        Apply RMSNorm to a 2D tensor: ::
            ...
            output = asctile.rms_norm(input, gamma, 1e-5)

        Apply RMSNorm to a 1D tensor: ::

            input = asctile.copy_in(x_gm, [0], [128])
            gamma = asctile.copy_in(gamma_gm, [0], [128])
            output = asctile.rms_norm(input, gamma, 1e-6)
    """
...
def layer_norm(input: LocalTensor, gamma: LocalTensor, beta: LocalTensor, epsilon: RuntimeFloat,
    ...
    builder = global_builder.get_ir_builder()
    mean_var_type = asctile.ir.get_asctile_LocalTensorType(mean_var_shape, builder.get_f32_type())
