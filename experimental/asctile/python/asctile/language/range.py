...
class range(BaseRange):
    ...
    In fact, built-in ``range`` automatically becomes ``asctile.range`` when used inside JIT function.
    ...
    Examples:
        ...
        Loop with unrolling: ::
            ...
            for i in asctile.range(0, N, 1, unroll_factor=4):
                ...

        Loop with a global memory barrier (prevent parallel memory optimizations): ::

            for i in asctile.range(N, gm_barrier=True):
                ...
    """
    ...
    def handle_op(self, op: ir.ForOp) -> None:
        ...
        op.set_attr(asctile.ir.attr.unroll_factor, builder.get_index_attr(self.unroll_factor))
        if self.gm_barrier:
            op.set_attr(asctile.ir.attr.gm_barrier, builder.get_unit_attr())
...
class static_range(asc_static_range):
    ...
    Examples:
        Loop with compile-time constant bounds: ::
            ...
            for i in asctile.static_range(0, 128):
                ...
    """
